# How to Build a Code-Editing Agent from Scratch in Go

> Build a 300-line Claude-powered agent that can read, list, and edit your files — no magic, just engineering.

![Hero - Code Editing Agent](images/go_ai_coding_hero.webp)

This guide walks you through building a real code-editing agent from zero, using Go and the Anthropic SDK. By the end, you'll have a terminal agent that can create and modify code on your filesystem.
...
Now, let's open `main.go` and put a skeleton of things we need in it:
...
Okay, let's move on, because the nicknames suck and this is not an agent yet.
...
![Tool Use Concept](images/tool_use_ai_flowchart.webp)
...
These models are trained and fine-tuned to use "tools" and they're very eager to do so. By now, 2025, they kinda "know" that they don't know everything and can use tools to get more information.
...
That's not much, is it? It's a single function, `ReadFile`, and two descriptions the model will see: our Description that describes the tool itself ("Read the contents of a given relative file path...") and a description of the single input parameter this tool has.
...
> **Note:** we return a list of strings and we denote directories with a trailing slash. That's not required, it's just something I just decided to do. There's no fixed format. Anything goes as long as Claude can make sense of it and whether it can you need to figure out by experimentation.
...
Right?! It's impressive, isn't it? And that's the most basic implementation of `edit_file` — of an agent in general — you can probably come up with.
...
It reads the file, it edits the file to change how long it runs, and then it also edits the file to update the comment at the top.
...
These models are incredibly powerful now. **300 lines of code and three tools and now you're able to talk to an alien intelligence that edits your code.** If you think "well, but we didn't really..." — go and try it! Go and see how far you can get with this. I bet it's a lot farther than you think.
